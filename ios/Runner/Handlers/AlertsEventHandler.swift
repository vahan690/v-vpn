...
public class AlertsEventHandler: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        // Send current alert state immediately when listener attaches
        let currentAlert = VPNManager.shared.alert
        DispatchQueue.main.async {
            var data: [String: Any?] = [
                "status": "Stopped",
                "alert": currentAlert.alert?.rawValue,
        ...
        // Then subscribe to future alert changes
