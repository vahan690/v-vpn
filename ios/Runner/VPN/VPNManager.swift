...
class VPNManager: ObservableObject {
    ...
    private func loadVPNPreference() async throws {
        let expectedBundleId = Bundle.main.baseBundleIdentifier + ".PacketTunnel"
        do {
            let managers = try await NETunnelProviderManager.loadAllFromPreferences()
            if let manager = managers.first {
                // Check if the existing config has the correct bundle identifier
                if let proto = manager.protocolConfiguration as? NETunnelProviderProtocol,
                   proto.providerBundleIdentifier == expectedBundleId {
                    self.manager = manager
                    print("[VPNManager] Using existing VPN configuration with bundle ID: \(expectedBundleId)")
                    return
                } else {
                    // Remove old configuration with wrong bundle identifier
                    print("[VPNManager] Removing old VPN configuration with incorrect bundle ID")
                    try await manager.removeFromPreferences()
                }
            }
            // Create new configuration
            print("[VPNManager] Creating new VPN configuration with bundle ID: \(expectedBundleId)")
            let newManager = NETunnelProviderManager()
            let `protocol` = NETunnelProviderProtocol()
            `protocol`.providerBundleIdentifier = expectedBundleId
            `protocol`.serverAddress = "localhost"
            newManager.protocolConfiguration = `protocol`
            newManager.localizedDescription = "V-VPN"
            try await newManager.saveToPreferences()
            try await newManager.loadFromPreferences()
            self.manager = newManager
            print("[VPNManager] VPN configuration created successfully")
        } catch {
            print("[VPNManager] Error: \(error.localizedDescription)")
        }
    }
