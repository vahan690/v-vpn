...
import 'package:vvpn/core/widget/adaptive_menu.dart';
import 'package:vvpn/features/common/confirmation_dialogs.dart';
import 'package:vvpn/features/common/qr_code_dialog.dart';
import 'package:vvpn/features/profile/model/profile_entity.dart';
import 'package:vvpn/features/profile/notifier/profile_notifier.dart';
import 'package:vvpn/features/profile/overview/profiles_overview_notifier.dart';
import 'package:vvpn/gen/fonts.gen.dart';
    ...
    return Card(
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            if ((profile is RemoteProfileEntity || !isMain) && profile.name != 'Unlock the World') ...[
              SizedBox(
                width: 48,
                child: Semantics(
...
class ProfileActionsMenu extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Hide actions for V-VPN profile ("Unlock the World")
    final isVvpnProfile = profile.name == 'Unlock the World';

    final menuItems = [
      if (profile case RemoteProfileEntity() when !isVvpnProfile)
        AdaptiveMenuItem(
